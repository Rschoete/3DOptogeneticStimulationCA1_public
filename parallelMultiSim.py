...
while FLAG:
    ...
    for i in range(numprocess):
        if FLAG and not busy_process_flags[i]:
            ...
            if not gifsim:
                subprocess.Popen(
                    ["runmultiSim.bat", rf"{myinput}", rf"{finishedpath}"], creationflags=subprocess.CREATE_NEW_CONSOLE)
            else:
                subprocess.Popen(
                    ["gifsim\\runmultiSimgif.bat", rf"{myinput}", rf"{finishedpath}"], creationflags=subprocess.CREATE_NEW_CONSOLE)
