...
if __name__ == '__main__':
    ...
    print(
        f"add paths: {SCRIPT_DIR}, {os.path.dirname(SCRIPT_DIR)} and {(os.path.dirname(os.path.dirname(SCRIPT_DIR)))} to path")
...
from Functions.globalFunctions.utils import Rx, Ry, Rz
    ...
    * **kwargs not used: add so no error when transfering kwargs from child classes
    ...
    def allign_cell_toaxis(self, axis='x', exclude_axon=True):
        ...
        try:
            ...
            xyz = np.array(
                [self.apic[-1].x3d(na), self.apic[-1].y3d(na), self.apic[-1].z3d(na)])
            angle = np.arccos(np.dot(xyz/np.linalg.norm(xyz), p1))
            phi = phi+np.pi if angle > np.pi/2 else phi
        except:
            # most of time morphology files are built with apical dendrites to the positive y-direction. correct if p1 is in negative y direction
            phi = phi+np.pi if phi < 0 else phi
        ...
        if phi != 0 or theta != 0 or psi != 0:
            for section in self.allsec:
                for i in range(section.n3d()):
                    xyz = np.array(
                        [section.x3d(i), section.y3d(i), section.z3d(i)])
                    R = np.dot(Rz(phi), np.dot(Ry(theta), Rx(psi)))
                    if inverse:
                        R = R.T
                    xyz = np.dot(R, xyz[:, None])
                    h.pt3dchange(i, xyz[0][0], xyz[1][0], xyz[2]
                                 [0], section.diam3d(i), sec=section)
        ...
        * set_pointer_xtra = True: create pointer between seg.extracellular._ref_Iopto and seg.extra_ox
        ...
        for sec in seclist:
            ...
            if set_pointer_xtra:
                for seg in sec:
                    h.setpointer(
                        getattr(seg, opsinmech)._ref_Iopto, 'ox', seg.xtra)
    ...
    def updateXtraCoors(self):
        ...
        for sec in self.allsec:
            if sec.has_membrane('xtra'):
                pt3ds = [[getattr(sec, x)(i) for i in range(sec.n3d())]
                         for x in ['x3d', 'y3d', 'z3d']]
                arcl = np.array([sec.arc3d(i) for i in range(sec.n3d())])
                arcl = arcl/arcl[-1]  # normalize
                for seg in sec:
                    xyz_seg = [np.interp(seg.x, arcl, coors)
                               for coors in pt3ds]
    ...
    def sec_plot(self, ax=None):
        ...
        colorlist = []
        colorkeyval = {'soma': 'tab:red', 'axon': 'tomato', 'apical trunk': 'tab:blue', 'apical trunk ext': 'royalblue',
                       'apical tuft': 'tab:green', 'apical obliques': 'tab:cyan', 'basal dendrites': 'tab:olive', 'unclassified': [0, 0, 0]}
        for sec in self.allsec:
            for seg in sec:
                if 'soma' in str(sec):
        ...
        mphv2.shapeplot(h, ax, sections=self.allsec,
                        cvals=colorlist, cb_flag=False, clim=[0, 0])
        ax.set_title(
            f"{self.templatename} / {self.morphology} / {self.celltype}")
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_zlabel('z')
                        ...
                        try:
                            ...
                            if autoset:
                                if 'was not made to point to anything' in E.args[0]:
                                    h.setpointer(
                                        getattr(seg.xtra, '_ref_'+y), x, seg.xtra)
                                else:
                                    print(E)
                            else:
                                raise E
    ...
    def updateMechValue(self, seglist, values, mech):
        ...
        if len(seglist) != len(values):
            raise ValueError(
                'seglist and values need to be same length or values needs to be float or integer')
                ...
                for sec in seclist:
                    for seg in sec:
                        values.append(distribution(
                            h.distance(source_hdistance, seg)))
                        seglist.append(seg)
            else:
    ...
    def calc_Gmax_mechvalue(self, mech, values=None, seglist='all'):
        ...
        if seglist == 'all':
            seglist = [
                seg for sec in self.allsec for seg in sec if hasattr(seg, mech)]
        if values is None:
        ...
        seglist, values = map(lambda x: list(x), zip(
            *filter(lambda x: hasattr(x[0], mech), zip(seglist, values))))
...
class CA1_PC_cAC_sig5(NeuronTemplate):
    ...
    def __init__(self, **kwargs):
        super().__init__(templatepath='./Model/cell_seed4_0-pyr-04.hoc',
                         templatename='CA1_PC_cAC_sig5', **kwargs)  # init of parent class
        self.load_template()
...
class CA1_PC_cAC_sig6(NeuronTemplate):
    def __init__(self, **kwargs):
        super().__init__(templatepath='./Model/cell_seed3_0-pyr-08.hoc',
                         templatename='CA1_PC_cAC_sig6', **kwargs)
        self.load_template()
        self.move_attributes()
        self.make_lists()
        self.moveSomaToOrigin()
...
class bACnoljp8(NeuronTemplate):
    def __init__(self, **kwargs):
        super().__init__(templatepath='./Model/cell_seed1_0-bac-10.hoc',
                         templatename='bACnoljp8', **kwargs)
        self.load_template()
        self.move_attributes()
        self.make_lists()
        self.moveSomaToOrigin()
...
class cNACnoljp1(NeuronTemplate):
    def __init__(self, **kwargs):
        super().__init__(templatepath='./Model/cell_seed1_0-cnac-04.hoc',
                         templatename='cNACnoljp1', **kwargs)
        self.load_template()
        self.move_attributes()
        self.make_lists()
        self.moveSomaToOrigin()
...
class bACnoljp7(NeuronTemplate):
    def __init__(self, **kwargs):
        super().__init__(templatepath='./Model/cell_seed2_0-bac-06.hoc',
                         templatename='bACnoljp7', **kwargs)
        self.load_template()
        self.move_attributes()
        self.make_lists()
        self.moveSomaToOrigin()
...
class cNACnoljp2(NeuronTemplate):
    def __init__(self, **kwargs):
        super().__init__(templatepath='./Model/cell_seed2_0-cnac-08.hoc',
                         templatename='cNACnoljp2', **kwargs)
        self.load_template()
        self.move_attributes()
        self.make_lists()
        self.moveSomaToOrigin()
...
class INT_cAC_noljp4(NeuronTemplate):
    def __init__(self, **kwargs):
        super().__init__(templatepath='./Model/cell_seed4_0-cac-06.hoc',
                         templatename='INT_cAC_noljp4', **kwargs)
        self.load_template()
        self.move_attributes()
        self.make_lists()
        self.moveSomaToOrigin()
...
class INT_cAC_noljp3(NeuronTemplate):
    def __init__(self, **kwargs):
        super().__init__(templatepath='./Model/cell_seed7_0-cac-04.hoc',
                         templatename='INT_cAC_noljp3', **kwargs)
        self.load_template()
        self.move_attributes()
        self.make_lists()
        self.moveSomaToOrigin()
...
if __name__ == '__main__':
    ...
    field = np.hstack(
        (data, 1000*myfun(data[:, 0], data[:, 1], data[:, 2], 1)[:, None]))
    structured = True
    # define norm => all plots have same colorscale
    norm = cm.colors.Normalize(
        vmax=abs(field[:, 3]).max(), vmin=field[:, 3].min())
    lognorm = cm.colors.LogNorm(
        vmax=abs(field[:, 3]).max(), vmin=field[field[:, 3] > 0, 3].min())

    # init figure and subplots
    fig, axs = plt.subplots(5, 5, figsize=(16, 9))

    poss = np.append(np.linspace(-50, -2, int((axs.size-2)/2), endpoint=False).round(
        2), np.linspace(0, 50, int(np.ceil((axs.size-2)/2)), endpoint=True).round(2))
    ...
    eF.slicePlot(field, axs, fig, (xX, xY, xZ), poss=poss, norm=norm,
                 xyz='xy', structured=True)
    fig, axs = plt.subplots(5, 5, figsize=(16, 9))
    eF.slicePlot(field, axs, fig, (xX, xY, xZ), poss=poss, norm=norm,
                 xyz='yz', structured=True)
    fig, axs = plt.subplots(5, 5, figsize=(16, 9))
    eF.slicePlot(field, axs, fig, (xX, xY, xZ), poss=poss, norm=lognorm,
                 xyz='xy', structured=True)

    # apply field to Neuron
    cell.updateXtraCoors()
    attach_flag, totalos = eF.attach_stim(
        cell.allsec, field, structured, netpyne=False, stimtype='optical', phi=np.pi/2, xT=[0, 0, -secpos['y2'].min()])
    ...
    swv, swov = cell.updateMechValue(
        seglist, valueschr2, mech='gchr2bar_chr2h134r')
    print(swov)
    ...
    if attach_flag:
        ...
        stim_time, stim_amp = eF.pulseTrain(
            t_pt, delay=delay, dur=dur, prf=prf, dc=dc, amp=amp)
    ...
    fig, axs = plt.subplots(
        2, 1, sharex=True, tight_layout=True, figsize=(9, 6))
    axs[0].plot(t, v_s)
    axs[1].plot(t, i_chr2)
