...
def calciOptogenx(input, t, traces):
    ...
    if input.analysesopt.recordTotalOptogeneticCurrent:
        if traces is None:
            print(
                'try to recordTotalOptogeneticCurrent but not traces recorded (traces is None)')
            return iOptogenx
        iOptogenx = {'abs': {'total': 0}, 'spec': {'total': 0}}
        tintm = np.array(t)
        idx = (tintm >= input.stimopt.Ostimparams.delay) & (
            tintm <= (input.stimopt.Ostimparams.delay+input.stimopt.Ostimparams.dur+1000))
...
def calcSuccesratio(input, t, spikeTimes, ostim_time, ostim_amp, estim_time, estim_amp):
    ...
    if input.analysesopt.recordSuccesRatio:
        ...
        sRopt['type'] = sRopt['type'] if isinstance(
            sRopt['type'], list) else [sRopt['type']]
        sRopt['window'] = sRopt['window'] if isinstance(
            sRopt['window'], list) else [sRopt['window']]
        sRopt['window'] = int(len(sRopt['type']) /
                              len(sRopt['window']))*sRopt['window']
        for sRtype, sRwindow in zip(sRopt['type'], sRopt['window']):
            if sRtype == 'eVstim':
                ...
                raise ValueError(
                    'input.analysesopt.succesRatioOptions["type"] can only be eVstim or Optogenx')
            ...
            # window for spike detection should always be smaller than start of next pulse
            t_off_spd = np.minimum(t_off+sRwindow, np.append(t_on[1:], t[-1]))
            intm = (sT >= t_on[:, None]) & (sT < t_off_spd[:, None])
            succesRatio['succes'][sRtype] = np.sum(np.any(intm, 1))/len(t_on)
            succesRatio['spikeCountperstimRatio'][sRtype] = np.sum(
                intm)/len(t_on)
