...
def SaveResults(input, cell, t, vsoma, traces, apcounts, aptimevectors, apinfo, totales, totalos, iOptogenx, succes_ratio, amps_SDeVstim, amps_SDoptogenx, pos_VTAeVstim, pos_VTAoptogenx, runtime, seed, results_dir):
    ...
    # this line last because inplace=False does not work -> always inplace
    inputData['settings'] = input.todict(inplace=False)
    if input.save_input_flag:
        with open(inputname, 'w') as outfile:
            json.dump(inputData, outfile, indent=4,
                      signif=input.signif,  cls=MyEncoder)

    # save results
    resultsname = results_dir+'/data.json'
    data = {}
    data['APs'] = addAPinfotoResults(
        apcounts, aptimevectors, apinfo) if apcounts is not None else None
    data['t'] = np.array(t) if t is not None else None
    data['vsoma'] = np.array(vsoma) if vsoma is not None else None
    data['traces'] = addTracestoResults(
        traces, input.samplingFrequency/input.analysesopt['samplefrequency_traces']) if traces is not None else None
    data['Optogxstim'] = addOSinfo(
        cell, input.cellsopt['opsin_options']['opsinmech'], input.stimopt['Ostimparams'])
    ...
    if input.save_data_flag:
        save_data_wCorrectSaveTest(
            resultsname, data, test_flag=test_flag, indent=4, signif=input.signif)
...
def addOSinfo(cell, opsinmech, stiminfo):
    ...
    for sec in cell.allsec:
        for seg in sec:
            out['gbar_opsin'].append(
                getattr(seg, f"gchr2bar_{opsinmech}") if hasattr(seg, opsinmech) else np.nan)
            out['segname'].append(str(seg))
            out['intensity'].append(
                seg.os_xtra if hasattr(seg, 'xtra') else np.nan)
...
def addESinfo(cell, stiminfo):
    ...
    for sec in cell.allsec:
        for seg in sec:
            out['segname'].append(str(seg))
            out['potential'].append(seg.es_xtra if (
                hasattr(seg, 'xtra') and hasattr(seg, 'extracellular')) else np.nan)
