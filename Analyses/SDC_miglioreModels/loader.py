...
def _recollect_data_df(*, filepath, result, all_columns, cell_init_options, settings_options, opsin_options, field_options, savename, save_recollect):
    ...
    for data_path, input_path in zip(alldata_files, allinput_files):
        ...
        cell_init_dict = {'neurontemplate': myinput['settings']['cellsopt']['neurontemplate'], **{key: myinput['settings']['cellsopt']['init_options'].get(key.rsplit(
            '_', 1)[0]) for key in cell_init_options if  not key in ['neurontemplate','hoc_file']}}
        cell_init_dict['hoc_file'] = myinput['settings']['cellsopt']['init_options'].get('hoc_file')
        
        if cell_init_dict['hoc_file'] is not None:
            if '/' in cell_init_dict['hoc_file']:
                cell_init_dict['hoc_file'] = cell_init_dict['hoc_file'].split('/')[-1]
            cell_init_dict['hoc_file'] = cell_init_dict['hoc_file'].rsplit('.')[0]
        settings_dict = {
...
if __name__ == '__main__':
    ...
    # Load data
    filepath = 'Results/SDC/SDC_constI_miglioreModels'  # './Results/SDC/SDC_constI_v2'
